...
# get diagnostics
#
diags = h5py.File(pathi+"diagnostics.h5","r")
time = diags['time'][:]*gamma
...
plt.plot(time,np.zeros_like(time),'--',color='0.5')
plt.plot(time,(diags['Work_q'][:])/(time/gamma)/POWER,label=r'$\varepsilon_q$')
...
plt.plot(time,np.zeros_like(time),'--',color='0.5')
plt.plot(time,diags['Work_w']/(time/gamma)/POWER,label=r'$\varepsilon_w$')
