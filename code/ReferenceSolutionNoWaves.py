"""
    Forced-disspative vertical plane-wave model: a reference waveless solution.
    ...
    Details:
        ...
        Wave forcing:     no forcing.

        Dissipation:      vorticity is damped through linear dissipation.
...
model = CoupledModel.Model(L=L,nx=nx, tmax = tmax,dt = dt, twrite=200,
                    nu4=0.,mu=mu,nu4w=0.,nu=0,nuw=0,muw=0*gamma,
...
remove_axes(ax)
plt.savefig('figs/energy_qg' , pad_inces=0, bbox_inches='tight')
...
remove_axes(ax)
plt.savefig('figs/energy_qg_anomaly' , pad_inces=0, bbox_inches='tight')
...
plt.ylabel(r"Power [$\dot \mathcal{P} \,/\, W$]")
plt.xlabel(r"$t\,\, \gamma$")
plt.savefig('figs/ke_budget_qg_generation' , pad_inces=0, bbox_inches='tight')
...
plt.savefig('figs/spectra_qg' , pad_inces=0, bbox_inches='tight')
