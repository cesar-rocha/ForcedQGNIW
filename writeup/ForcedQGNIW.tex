...
\begin{table}
 \begin{center}
   \caption{Wave potential energy budget of the reference and no-drag solutions.
              ...
              of wave potential energy via stimulated generation.}
   \label{P-budgets}
   \begin{tabular}{ l | c | c }
     \hline
       &  Reference & No-drag  \\
...
action density for a solution with $\sigma_w = 4 \sigma_q $ and $\gamma = 4\mu$; table
\ref{parameters_reference} describes in detail the parameters of
this reference solution. (The parameters yield a solution in the asymptotic
regime of valid of the model.)  The equilibrated potential vorticity in figure
...
of this wave-modified turbulence is more fine-grained.
...
The snapshot in figure \ref{snapshots_pv_qg-niw_reference}b resembles the wave field in decaying wave-modified
two-dimensional turbulence \citep{rocha_etal2017}.
...
\section{Solution with $\mu = 0$}
A natural question is whether the system equilibrates without bottom drag. We thus
solve the vertical plane-wave model with $\mu = 0$; all other parameters are the
same as in the reference solution above. The balanced kinetic energy did not
equilibrate over course of 80 wave dissipative time units (see figure \ref{energies_nodrag}).
But the waves do equilibrate.

The snapshot of potential vorticity at $t\gamma = 60$ (figure \ref{snapshots_pv_qg-niw_nodrag})
shows weaker large-scale vortices. These vortices are bigger than the the vortices
of the reference solution with $\mu\neq 0$ but also less coherent. Thus, the balanced
kinetic energy has cascade towards larger scales. The wave action density is
concentrated near the big vortices and has much finer scales than the $\mu\neq 0$
solution above.

By $t\gamma = 60$ the balanced kinetic energy $\K$ is about the same times larger
than the equilibrated wave kinetic energy $f_0\A$. Wave potential energy $\P$
appears to equilibrate at $10\%$ of $f_0 \A$.

The wave action budget remains the same: the action delivered by the the stochastic
forcing is removed by linear dissipation of $\phi$. The wave potential energy budget
is also as simple as before: $\P$ is created by stimulated generation and removed
by linear dissipation. And geostrophic straining accounts for most of stimulated
generation.

The balanced kinetic energy budget differs markedly from the $\mu\neq 0$ solution.
Wave streaming accounts for about 30-40\% of the generation of $\K$; all the input energy
is removed by stimulated generation.


\begin{figure}
\centering
\includegraphics[width=.925\textwidth]{figs/snapshots_pv_qg-niw_nodrag.png}
\caption{Snapshot of potential vorticity and wave action density for the solution
         with parameters in  table \ref{parameters_reference} but $\mu=0$. The scale of potential vorticity
         and action are  the same as in figure 1.}
        \label{snapshots_pv_qg-niw_nodrag}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=.825\textwidth]{figs/energies_nodrag.png}
\caption{(a) Balanced kinetic energy ($\K$) and wave potential energy ($\P$) and wave
         kinetic energy ($f_0 \A$)  for the solution with parameters in table
         \ref{parameters_reference}. The energy difference, $\Delta \K$ and $\Delta \P$,
         about a time average after equilibration ($t\,\gamma \ge 5$).}
        \label{energies_nodrag}
\end{figure}
